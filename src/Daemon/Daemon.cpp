...
void ExecMainLoop()
{
	...
	// Context
	std::vector<DaemonHelper::InjectedClient> activeClients{ };
	size_t crashCounter{ };

	while (crashCounter < k_maxTolerableCrashes)
	{
		DaemonHelper::SleepAdaptively(activeClients);

		DaemonHelper::RemoveTerminatedClients(activeClients);
		const auto injectionSummary = DaemonHelper::PatchCandidateClients(payload.value(), activeClients);
		crashCounter = DaemonHelper::UpdatedCrashCounter(crashCounter, injectionSummary);
	}
